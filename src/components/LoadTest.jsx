...
const LoadTest = ({ url, urlID, queriesList }) => {
  ...
  const queries = [];
  queriesList.map((prevQuery, index) =>
    queries.push(
      <option
  ...
  const sendQuery = () => {
    ...
    window.api.receiveArray(
      "loadTestResultsFromMain",
      (event, loadTestResults) => {
        setavgResponseTime(
          loadTestResults[
            loadTestResults.length - 1
          ].average_response_time.toFixed(1)
        );
  ...
  return (
    <div id="test-query" style={themeStyle}>
      <header className="uri">
        ...
        <select name="queries-list" id="queries-list" onChange={handleChange}>
      ...
      <div id="stats">
        <div className="category">
          <div className="category-title">
            Avg Response Time for {loadAmount} Requests
          </div>
