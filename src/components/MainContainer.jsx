...
const MainContainer = ({ userID, urlList, setUrlList }) => {
  ...
  const getResponseTimes = () => {
    window.api.receiveArray("responseTimesFromMain", (event, arg) => {
      ...
      const responseTimeSum = arg.reduce(
        (sum, curr) => (sum += curr.response_time),
        0
      );
      ...
      <Router>
        ...
        <Switch>
          <Route exact path="/dashboard">
            <Dashboard url={url} urlID={urlID}></Dashboard>
          </Route>
          <Route exact path="/testquery">
            <TestQuery
              url={url}
              urlID={urlID}
              history={history}
