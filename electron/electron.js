...
ipcMain.on('dashboardToMain', async (event, arg) => {
  ...
  const result4 = await query(numberOfLoadTestSuccesses);
  const result5 = await query(numberOfLoadTestFailures);
  const result6 = await query(allQueries);
  const result7 = await query(allLoadTests);
